...
const server = http.createServer(async (req, res) => {
  ...
  // GET - Buscar desafio por ID
    ...
    const id = Number.parseInt(req.url.split("/").pop())

    const { data, error } = await supabase.from("desafio").select("*").eq("id", id).single()
  ...
  // PUT - Editar desafio
    ...
    req.on("end", async () => {
      ...
      const { data, error } = await supabase.from("desafio").update({ titulo, descricao, id_categoria }).eq("id", id)
  ...
  // DELETE - Marcar desafio como inativo
    ...
    const { data, error } = await supabase.from("desafio").update({ ativo: false }).eq("id", id)
    ...
    try {
      const { data, error } = await supabase.rpc("listar_usuarios_com_tipo")
    ...
    req.on("end", async () => {
      try {
        ...
        res.writeHead(201, { "Content-Type": "application/json" })
        return res.end(JSON.stringify({ sucesso: true, usuario: data }))
      } catch (err) {
    ...
    try {
      const { data, error } = await supabase.from("categoria_desafio").select("id, nome").eq("ativo", true)
    ...
    req.on("end", async () => {
      try {
        ...
        const { data, error } = await supabase.from("categoria_desafio").insert([{ nome, ativo: true }])
  ...
  // PUT atualizar categoria
    ...
    req.on("end", async () => {
      const { nome } = JSON.parse(body)
      const { error } = await supabase.from("categoria_desafio").update({ nome }).eq("id", id)
  ...
  // DELETE (desativar)
    ...
    const id = req.url.split("/").pop()
    const { error } = await supabase.from("categoria_desafio").update({ ativo: false }).eq("id", id)
